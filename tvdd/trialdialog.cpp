...
nlohmann::json TTrialDialog::DialogToJSON()
{
	...
	sav[str_ofSelectable] = tr_ofSelectable;
	sav[str_ofTopSelect] = tr_ofTopSelect;
	sav[str_ofFirstClick] = tr_ofFirstClick;
	sav[str_ofFramed] = tr_ofFramed;
	sav[str_ofPreProcess] = tr_ofPreProcess;
	sav[str_ofPostProcess] = tr_ofPostProcess;
	sav[str_ofBuffered] = tr_ofBuffered;
	sav[str_ofTileable] = tr_ofTileable;
	sav[str_ofCenterX] = tr_ofCenterX;
	sav[str_ofCenterY] = tr_ofCenterY;
...
void TTrialDialog::GenCode(ofstream* res)
{
	...
	*res << " TWindowInit(&" << base_class_name << "::initFrame)\n{\n";
	...
	if (tr_ofTopSelect)
		*res << " options |= ofTopSelect;\n";
	if (tr_ofFirstClick)
		*res << " options |= ofFirstClick;\n";
	if (tr_ofFramed)
		*res << " options |= ofFramed;\n";
	if (tr_ofPreProcess)
		*res << " options |= ofPreProcess;\n";
	if (tr_ofPostProcess)
		*res << " options |= ofPostProcess;\n";
	if (tr_ofBuffered)
		*res << " options |= ofBuffered;\n";
	if (tr_ofTileable)
		*res << " options |= ofTileable;\n";
	if (tr_ofCenterX)
		*res << " options |= ofCenterX;\n";
	if (tr_ofCenterY)
		*res << " options |= ofCenterY;\n";
	if (tr_ofValidate)
		*res << " options |= ofValidate;\n";
