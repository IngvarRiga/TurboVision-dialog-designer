...
Всё нижеописанное находится в тестировании в собственных разработках, если будут ошибки буду правит/менять.
...
---
### 28.06.2023
- [x] Правка редактора свойств TInputDouble;
- [x] TInputDouble теперь содержит свойство AllowNotDefined, определяющее, что компонент может содержать неопределённое значение. Неопределённое значение задаётся как пустая строка, при этом компонент рисуется тёмно-серым цветом и выводятся допустимые границы задания значений. Соответсвенно, добавлены функции управления этим свойством;
- [x] В палитру компонентов добавлен TMemo (редактор свойств пока не функционален);
- [x] В палитру компонентов добавлен TListBox (редактор свойств пока отсутствует);
- [x] Исправлена функция удалить в выпадающем меню для TInputLong / TInputDouble;
- [x] TInputDouble установлены ограничения по умолчанию от -DBL_MAX до DBL_MAX. В Microsoft всё работает, в gcc 13.1 что-то не так с определениями констант;
- [x] исправлена сборка cmake для MinGW 64 (в который раз);

### 22.06.2023
- [x] Правка редактора свойств TInputLong;
- [x] TInputLong теперь содержит свойство AllowNotDefined, определяющее, что компонент может содержать неопределённое значение. Неопределённое значение задаётся как пустая строка, при этом компонент рисуется тёмно-серым цветом и выводятся допустимые границы задания значений. Соответсвенно, добавлены функции управления этим свойством;

### 12.06.2023
- [x] Правка cmakelists.txt;
- [x] Откорректирован код в части подавления предупреждений компиляции;

### 05.06.2023
- [x] textrainput.h / textinputlong(double) - содержат определения для ввода long (double) с заданными ограничениями. Если вводимое значение не допустимо строка подсвечивается красным. Если вводимое значение пустое - выводится диапазон допустимых значений (по умолчанию - очень длинный текст и может не помещаться в строку). Правый клик на строке ввода - контектное меню с возможностью выбора 3-х значений минимума / по умолчанию / максимума. Имеют свойства setValue() / getValue() которыми нужно пользоваться для установки и получения значений.
- [x] Исправлен авторазмер диалога;
- [x] TInputDouble - добавлено свойство 'точность' - отвечает за точность представления числа, когда пользователь устанавливает значение программно. Не влияет на точность представления при ручном вводе числа;
- [x] Добавлено окно редактирования свойств TInputDouble;
- [x] Исправлена генерация кода диалога в части TInputDouble;
- [x] Английские ресурсы для окна свойств TInputLong;

### 01.06.2023
- [x] Оптимизация кода;
- [x] Сделан и проверен cmake для сборки (немного корявый, но как есть);
- [x] msvc_eng (_ru) - командные файлы для полной сборки (Release) для MS Visual C++ с соответствующими ресурсами;

### 31.05.2023
- [x] Оптимизация кода;
- [x] Перевод окна свойств TDialog;
