...
- [x] TInputDouble теперь содержит свойство AllowNotDefined, определяющее, что компонент может содержать неопределённое значение. Неопределённое значение задаётся как пустая строка, при этом компонент рисуется тёмно-серым цветом и выводятся допустимые границы задания значений. Соответственно, добавлены функции управления этим свойством;
